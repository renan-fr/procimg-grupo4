...
def read_image_any(input_bytes: bytes | None, path: Path | None) -> tuple[np.ndarray | None, str]:
    """Lê imagem de upload (bytes) ou de arquivo (path). Retorna (img_bgr, origem)."""
    if input_bytes:
...
def pil_to_bgr(img_any):
    """Converte PIL.Image ou array RGB/GRAY para BGR uint8 (OpenCV)."""
    try:
...
def first_ndarray(*vals):
    """Pega o primeiro ndarray ou PIL.Image e normaliza pra ndarray BGR."""
    for v in vals:
        if v is None:
            continue
...
with left:
    ...
    # Preferência: upload ou arquivo de 'entradas/'
...
with right:
    ...
    # Widgets de parâmetros por operação
    a = SimpleNamespace()
    ...
    if op == "mapear-cores":
        ...
        a.cor = hex_to_bgr(cor_hex)  # (B,G,R)
        ...
        cor_hex = st.color_picker("Cor alvo", value="#FF0000")
        a.cor = hex_to_bgr(cor_hex)
        a.fator = st.slider("Fator (0-1)", 0.0, 1.0, 0.5, step=0.05)
        ...
        # OpenCV trabalha com Hue em 0–179
    ...
    if run_it:
        if img is None:
            ...
            try:
                ...
                # se vier tuple/list, inclui todos para tentar extrair o primeiro válido
                if isinstance(out_raw, (tuple, list)):
                    candidatos.extend(list(out_raw))

                # pega o primeiro ndarray (ou PIL convertido) válido
                ...
                # Checagem leve: a saída realmente mudou?
                ...
                # Salvar arquivo em 'saidas/<stem>__<op>.png'
